...
# :: (sym, str, str) -> str
def dump_cmd_str(db_sym, collection_name=nil, query_doc=nil)
  db_data = DBS[db_sym]
...
def get_mongo_collection(db_sym, collection_name)
...
def get_id_for_nth_most_recent_doc(collection, n)
...
def dump_docs_after_id(db_sym, collection_name, doc_id)
...
def dump_last_n_docs(db_sym, collection_name, n)
...
def dump_remote()
  ...
  dump_last_n_docs(:areas, 'areas', 1_000)
  %w(learn_contents lists locations).each do |collection_name|
...
def restore_db_to_local(db_sym)
